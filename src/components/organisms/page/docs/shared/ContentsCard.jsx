...
const ContentsCard = ({ accentColor, children, topics, logoComponent: SvgLogo, svgType, title, ...passProps }) => {
  const renderedTopics = topics.map(({ url, title: topicTitle, iconComponent: SvgIcon, iconOutlined, svgType: topicIconSvgType }) => (
    <Topic key={topicTitle} as={url && A} href={url} inProgress={!url}>
      <TopicContent>
        <TopicIcon accentColor={accentColor} iconComponent={SvgIcon} outlined={iconOutlined} svgType={topicIconSvgType} />
